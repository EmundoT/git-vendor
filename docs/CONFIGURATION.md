...
> **Authoritative source:** [`CLAUDE.md`](../CLAUDE.md) — the project primer contains the complete, maintained configuration documentation including all fields, interfaces, and data models.
>
> This file covers vendor.yml and vendor.lock formats with examples and validation rules. For the full data model including all schema versions, see `internal/types/types.go`.
...
# Cascade config (optional)
cascade:
  root: ..                          # Parent directory containing sibling repos
  verify: true                      # Run build/test after each pull
  commit: true                      # Auto-commit after each pull

vendors:
  - name: string                    # Required
    url: string                     # Required (or source: internal)
    mirrors: []string               # Optional (schema v1.3+)
    source: string                  # Optional: "internal" for same-repo vendors (Spec 070)
    ...
    policy:                         # Optional per-vendor policy override
      block_on_drift: true
      block_on_stale: true
      max_staleness_days: 7
    exclude: []string               # Optional: glob patterns to skip during sync
    hooks:                          # Optional
      pre_sync: string
      post_sync: string
    specs:                          # Required (≥1)
      - ref: string                 # Required (use "local" for internal vendors)
...
vendors:
  - name: string
    ...
    file_hashes:                    # path -> SHA-256 hash
      path/to/file: "sha256:..."
    # Metadata (v1.1+)
    license_spdx: string
    source_version_tag: string
    vendored_at: string (ISO8601)
    vendored_by: string
    last_synced_at: string (ISO8601)
    # Position extraction (v1.2+)
    positions: []                   # Position-extracted mappings
    # Multi-remote (v1.3+)
    source_url: string              # Which URL served content (empty = primary)
    # Accepted drift (CLI-003)
    accepted_drift:                 # path -> SHA-256 of accepted local content
      path/to/file: "sha256:..."
    # Internal vendors (Spec 070)
    source: "internal"
    source_file_hashes:             # source path -> SHA-256
      src/path: "sha256:..."
```

For the full schema history, see `internal/types/types.go` (VendorLock, LockDetails).
...
vendors:
  - name: example-lib
    ...
    file_hashes:
      internal/utils/strings.go: "sha256:7e39ab..."
    vendored_at: "2024-12-20T08:00:00Z"
    vendored_by: "user <user@example.com>"
    last_synced_at: "2024-12-27T10:30:45Z"
```

**Never edit this file manually.** It's automatically generated by `git-vendor pull`, `git-vendor accept`, and related commands.
...
- [CLAUDE.md](../CLAUDE.md) - Authoritative project primer (commands, architecture, data model)
- [Commands Reference](./COMMANDS.md) - Command index (redirects to CLAUDE.md)
